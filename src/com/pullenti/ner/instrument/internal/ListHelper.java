...
public class ListHelper {
                ...
                for (j = i + 1; j < res.children.size(); j++) {
                    ...
                    if (ch.getEndToken().isCharOf(":")) {
                        if (((int)listBullet) != 0 && ch.getBeginToken().isChar(listBullet)) {
                            for (com.pullenti.ner.Token tt = ch.getBeginToken().getNext(); tt != null && (tt.endChar < ch.endChar); tt = tt.getNext()) {
                                if (tt.getPrevious().isChar('.') && com.pullenti.ner.core.MiscHelper.canBeStartOfSentence(tt)) {
                                    FragToken ch2 = FragToken._new1377(tt, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, ch.number);
    ...
    private static int _analizeListItems(java.util.ArrayList<FragToken> chi, int ind) {
        ...
        if (res.getHasChanges() && res.getMultilineChangesValue() != null) {
            ...
            FragToken cit = FragToken._new1360(ci.getBeginToken(), ci.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CITATION);
            ...
            if (citKind != com.pullenti.ner.instrument.InstrumentKind.UNDEFINED && citKind != com.pullenti.ner.instrument.InstrumentKind.APPENDIX) {
                ...
                sub = FragToken.createDocument(ci.getBeginToken(), ci.endChar, citKind);
        ...
        if (res.itok == null) 
            res.itok = InstrToken1.parse(res.getBeginToken(), true, null, 0, null, false, res.endChar, false, false);
        ...
        if (res.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
            for (int j = ind + 1; j < chi.size(); j++) {
                if (chi.get(j).kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                    java.util.ArrayList<LineToken> lines2 = LineToken.parseList(chi.get(j).getBeginToken(), chi.get(j).endChar, lines.get(lines.size() - 1));
        ...
        for (LineToken li : lines) {
            ...
            FragToken ch = FragToken._new1377(li.getBeginToken(), li.getEndToken(), (li.isListItem ? com.pullenti.ner.instrument.InstrumentKind.LISTITEM : com.pullenti.ner.instrument.InstrumentKind.CONTENT), li.number);
            ...
            if (chli != null) {
                for (LineToken lt : chli) {
                    ch.children.add(FragToken._new1360(lt.getBeginToken(), lt.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.LISTITEM));
                }
                if (ch.beginChar < ch.children.get(0).beginChar) 
                    ch.children.add(0, FragToken._new1360(ch.getBeginToken(), ch.children.get(0).getBeginToken().getPrevious(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
            }
        }
        return ret;
    }
    ...
    public static class LineToken extends com.pullenti.ner.MetaToken {
        ...
        public static LineToken parse(com.pullenti.ner.Token t, int maxChar, LineToken prev) {
            ...
            for (; t != null && t.endChar <= maxChar; t = t.getNext()) {
                if (t.isChar(':')) {
            ...
            if ((lines.get(i).typ == InstrToken1.Types.LINE && lines.get(i).numbers.size() == 0 && lines.get(i).getBeginToken().isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) && lines.get(i + 1).numbers.size() > 0 && lines.get(i).getEndToken().isChar(':')) {
                ...
                for (++i; i < lines.size(); i++) {
                    if (lines.get(i).numbers.size() == 0) {{
                        ...
                        int nn;
                        com.pullenti.unisharp.Outargwrapper<Integer> wrapnn1566 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                        boolean inoutres1567 = com.pullenti.unisharp.Utils.parseInteger(lines.get(i).numbers.get(0), 0, null, wrapnn1566);
                        nn = (wrapnn1566.value != null ? wrapnn1566.value : 0);
                        if (inoutres1567) 
                            num = nn + 1;
    ...
    public static void correctIndex(java.util.ArrayList<InstrToken1> lines) {
        ...
        if (index.size() == content.size() && index.size() > 2) {
            if ((indText * 10) < conText) {
                com.pullenti.unisharp.Utils.putArrayValue(lines, 0, InstrToken1._new1568(lines.get(0).getBeginToken(), lines.get(cInd - 1).getEndToken(), true, InstrToken1.Types.INDEX));
    ...
    private static boolean _canBeEquals(InstrToken1 i1, InstrToken1 i2) {
        ...
        if (!com.pullenti.ner.core.MiscHelper.canBeEqualsEx(i1.value, i2.value, com.pullenti.ner.core.CanBeEqualsAttrs.of((com.pullenti.ner.core.CanBeEqualsAttrs.IGNORENONLETTERS.value()) | (com.pullenti.ner.core.CanBeEqualsAttrs.IGNOREUPPERCASE.value())))) 
            return false;
        return true;
    }
    public ListHelper() {
    ...
    static {
        _globalInstance = new ListHelper(); 
    }
}
