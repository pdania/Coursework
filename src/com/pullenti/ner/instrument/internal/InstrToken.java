...
public class InstrToken extends com.pullenti.ner.MetaToken {
    ...
    public static void initialize() {
        ...
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new130("ФАМИЛИЯ ИМЯ ОТЧЕСТВО", "ФИО");
        t.addAbridge("Ф.И.О.");
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new1523("ПРІЗВИЩЕ ІМЯ ПО БАТЬКОВІ", com.pullenti.morph.MorphLang.UA, "ФИО");
        m_Ontology.add(t);
    ...
    public boolean isPurePerson() {
        if (ref instanceof com.pullenti.ner.ReferentToken) {
            ...
            if ((rt.referent instanceof com.pullenti.ner.person.PersonReferent) || (rt.referent instanceof com.pullenti.ner.person.PersonPropertyReferent)) 
                return true;
            if (rt.referent instanceof com.pullenti.ner.instrument.InstrumentParticipant) {
                for (com.pullenti.ner.Token t = rt.getBeginToken(); t != null && t.endChar <= rt.endChar; t = t.getNext()) {
    ...
    public static java.util.ArrayList<InstrToken> parseList(com.pullenti.ner.Token t0, int maxChar) {
        ...
        for (com.pullenti.ner.Token t = t0; t != null; t = t.getNext()) {
            ...
            if (res.size() > 0) {
                if (res.get(res.size() - 1).endChar > it.beginChar) 
                    break;
            }
            ...
            if (it.typ == ILTypes.UNDEFINED && t.isNewlineBefore()) {
                ...
                if (it1 != null && it1.getHasChanges() && it1.endChar > it.endChar) 
    ...
    private static InstrToken _correctPerson(InstrToken res) {
        ...
        if (rt0 != null && (rt0.referent instanceof com.pullenti.ner.instrument.InstrumentParticipant)) {
            for (com.pullenti.ner.Token tt = res.getBeginToken(); tt != null && tt.endChar <= res.endChar; tt = tt.getNext()) {
                if ((tt.getReferent() instanceof com.pullenti.ner.person.PersonReferent) || (tt.getReferent() instanceof com.pullenti.ner.person.PersonPropertyReferent)) {
                    ...
                    for (com.pullenti.ner.Token ttt = (((com.pullenti.ner.MetaToken)com.pullenti.unisharp.Utils.cast(tt, com.pullenti.ner.MetaToken.class))).getBeginToken(); ttt != null && ttt.endChar <= tt.endChar; ttt = ttt.getNext()) {
    ...
    public static InstrToken parse(com.pullenti.ner.Token t, int maxChar, InstrToken prev) {
        ...
        for (; t != null; t = t.getNext()) {
            ...
            if (isStartOfLine && t == t0) {
                ...
                if ((t.getReferent() instanceof com.pullenti.ner.person.PersonReferent) || (t.getReferent() instanceof com.pullenti.ner.person.PersonPropertyReferent) || (t.getReferent() instanceof com.pullenti.ner.instrument.InstrumentParticipant)) 
                    return _correctPerson(_new1524(t00, t, ILTypes.PERSON, t));
                boolean isRef = false;
                if (t.getReferent() instanceof com.pullenti.ner.person.PersonPropertyReferent) {
                    ...
                    com.pullenti.ner.ReferentToken rt = t.kit.processReferent(com.pullenti.ner.person.PersonAnalyzer.ANALYZER_NAME, t);
                    if (rt != null) {
                        if (rt.referent instanceof com.pullenti.ner.person.PersonReferent) 
                            return _new1525(t00, rt.getEndToken(), ILTypes.PERSON);
                ...
                for (; tt != null; tt = tt.getNext()) {
                    ...
                    if (re instanceof com.pullenti.ner.person.PersonReferent) 
                        return _new1524(t00, tt, ILTypes.PERSON, tt);
                ...
                if (tt == null && isRef) 
                    return _new1524(t00, (t11 != null ? t11 : t), ILTypes.PERSON, t);
            }
            ...
            if (dt != null) {
                if (dt.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.TYP && !t.chars.isAllLower()) {
                    ...
                    if (!_hasVerb) {
                        InstrToken res2 = _new1528(t0, dt.getEndToken(), ILTypes.TYP, (dt.fullValue != null ? dt.fullValue : dt.value));
                        if (com.pullenti.unisharp.Utils.stringsEq(res2.value, "ДОПОЛНИТЕЛЬНОЕ СОГЛАШЕНИЕ") || com.pullenti.unisharp.Utils.stringsEq(res2.value, "ДОДАТКОВА УГОДА")) {
                            if (res2.beginChar > 500 && res2.getNewlinesBeforeCount() > 1) 
                ...
                if (dt.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.NUMBER) {
                    if (t != t0) 
                        break;
                    return _new1528(t0, dt.getEndToken(), ILTypes.REGNUMBER, dt.value);
                }
                if (dt.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.ORG) {
                    if (t != t0) 
                        break;
                    return _new1530(t0, dt.getEndToken(), ILTypes.ORGANIZATION, dt.ref, dt.value);
                }
                if (dt.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.TERR) {
                    if (t != t0) 
                        break;
                    InstrToken re = _new1530(t0, dt.getEndToken(), ILTypes.GEO, dt.ref, dt.value);
                ...
                if (dt.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.OWNER) {
                    ...
                    if (dt.ref != null && dt.ref.referent.toString().startsWith("агент")) 
                        dt = null;
                    if (dt != null) {
                        InstrToken res1 = _new1530(t0, dt.getEndToken(), ILTypes.PERSON, dt.ref, dt.value);
            ...
            if (r instanceof com.pullenti.ner.date.DateReferent) {
                ...
                if (!hasWord) 
                    return _new1524(t, tt, ILTypes.DATE, t);
                if (t != t0) 
                    break;
            }
            hasWord = true;
            if (r instanceof com.pullenti.ner.instrument.InstrumentParticipant) {
                for (com.pullenti.ner.Token tt = (((com.pullenti.ner.ReferentToken)com.pullenti.unisharp.Utils.cast(t, com.pullenti.ner.ReferentToken.class))).getBeginToken(); tt != null && (tt.endChar < t.endChar); tt = tt.getNext()) {
            ...
            if ((r instanceof com.pullenti.ner.person.PersonReferent) || (r instanceof com.pullenti.ner.person.PersonPropertyReferent) || (r instanceof com.pullenti.ner.instrument.InstrumentParticipant)) {
                if (t != t0) 
                    break;
                if (r instanceof com.pullenti.ner.instrument.InstrumentParticipant) {
                }
                InstrToken res1 = _new1524(t, t, ILTypes.PERSON, t);
                return _correctPerson(res1);
            }
            if (r instanceof com.pullenti.ner._org.OrganizationReferent) {
                if (t != t0) 
                    break;
                return _new1524(t, t, ILTypes.ORGANIZATION, t);
            }
                ...
                if (dpr.getAppendix() != null) {
                    if (t.isNewlineBefore() || isStartOfLine) {
                        if (t.isNewlineAfter() || t.getWhitespacesBeforeCount() > 30) 
                            return _new1528(t, t, ILTypes.APPENDIX, "ПРИЛОЖЕНИЕ");
                        boolean ok = true;
            ...
            if ((r instanceof com.pullenti.ner.decree.DecreeReferent) && (((com.pullenti.ner.decree.DecreeReferent)com.pullenti.unisharp.Utils.cast(r, com.pullenti.ner.decree.DecreeReferent.class))).getKind() == com.pullenti.ner.decree.DecreeKind.PUBLISHER && t == t0) {
                InstrToken res1 = _new1525(t, t, ILTypes.APPROVED);
                for (com.pullenti.ner.Token tt = t.getNext(); tt != null; tt = tt.getNext()) {
                    if (tt.isCharOf(",;")) 
                        continue;
            ...
            if (t.isValue("ЗА", null) && t.getNext() != null && t.isNewlineBefore()) {
                ...
                if ((rr instanceof com.pullenti.ner.person.PersonReferent) || (rr instanceof com.pullenti.ner.person.PersonPropertyReferent) || (rr instanceof com.pullenti.ner.instrument.InstrumentParticipant)) {
                    if (t != t0) 
                        break;
                    InstrToken res1 = _new1524(t, t.getNext(), ILTypes.PERSON, t.getNext());
                    t = t.getNext().getNext();
                    if ((rr instanceof com.pullenti.ner.instrument.InstrumentParticipant) && t != null) {
            ...
            for (int ii = 0; ii < m_Directives.size(); ii++) {
                if (t.isValue(m_Directives.get(ii), null)) {
                    if (t.getNext() != null && t.getNext().isValue("СЛЕДУЮЩЕЕ", "НАСТУПНЕ")) {
                        ...
                        if (ok) 
                            return _new1528(t, t11, ILTypes.DIRECTIVE, m_DirectivesNorm.get(ii));
                    }
                    if (t.isNewlineAfter() || ((t.getNext() != null && t.getNext().isChar(':') && t.getNext().isNewlineAfter()))) {
                        ...
                        return _new1528(t, (t.isNewlineAfter() ? t : t.getNext()), ILTypes.DIRECTIVE, m_DirectivesNorm.get(ii));
                    }
                    break;
                }
            }
                ...
                for (String d : m_Directives) {
                    ...
                    if (t11 != null) {
                        if (t11.getNext() != null && t11.getNext().isChar(':')) 
                            t11 = t11.getNext();
                        return _new1525(t, t11, ILTypes.DIRECTIVE);
                    }
                }
            }
            ...
            if (isStartOfLine && !tte.chars.isAllLower() && t == t0) {
                com.pullenti.ner.core.NounPhraseToken npt = com.pullenti.ner.core.NounPhraseHelper.tryParse(tte, com.pullenti.ner.core.NounPhraseParseAttr.NO, 0);
                ...
                if (npt != null && npt.getMorph().getCase().isNominative() && (npt.getEndToken() instanceof com.pullenti.ner.TextToken)) {
                    ...
                    if (((com.pullenti.unisharp.Utils.stringsEq(term1, "ПРИЛОЖЕНИЕ") || com.pullenti.unisharp.Utils.stringsEq(term1, "ДОДАТОК") || com.pullenti.unisharp.Utils.stringsEq(term1, "МНЕНИЕ")) || com.pullenti.unisharp.Utils.stringsEq(term1, "ДУМКА") || com.pullenti.unisharp.Utils.stringsEq(term1, "АКТ")) || com.pullenti.unisharp.Utils.stringsEq(term1, "ФОРМА") || com.pullenti.unisharp.Utils.stringsEq(term, "ЗАЯВКА")) {
                        ...
                        if (tt1 == null) 
                            ...
                            return _new1528(t, t, ILTypes.APPENDIX, term1);
                        ...
                        if ((ok && t.getPrevious() != null && (t.getNewlinesBeforeCount() < 3)) && !t.isNewlineAfter()) {
                            ...
                            for (com.pullenti.ner.Token ttp = t.getPrevious(); ttp != null; ttp = ttp.getPrevious()) {
                                if (!ttp.isNewlineBefore()) 
                                    continue;
                                for (; ttp != null && (ttp.endChar < t.beginChar); ttp = ttp.getNext()) {
            ...
            if (app) {
                for (com.pullenti.ner.Token tt = t.getNext(); tt != null; tt = tt.getNext()) {
                    if (tt.isNewlineBefore()) 
                        break;
                    else if (com.pullenti.morph.MorphClass.ooEq(tt.getMorphClassInDictionary(), com.pullenti.morph.MorphClass.VERB)) {
                        app = false;
                        break;
                    }
                }
                if (app) 
                    return _new1525(t, t.getNext(), ILTypes.APPENDIX);
            }
            if (!t.chars.isAllLower() && t == t0) {
                ...
                if (tt != null) {
                    if (tt.getNext() != null && (tt.getNext().getReferent() instanceof com.pullenti.ner.decree.DecreeReferent)) 
                        return _new1524(t, tt, ILTypes.APPROVED, tt.getNext().getReferent());
                    ...
                    if (dt1 != null && dt1.typ == com.pullenti.ner.decree.internal.DecreeToken.ItemType.TYP) 
                        return _new1525(t, tt, ILTypes.APPROVED);
                }
            }
            t1 = t;
            isStartOfLine = false;
        ...
        if (t1 == null) 
            return null;
        InstrToken res = _new1525(t00, t1, ILTypes.UNDEFINED);
        res.noWords = true;
    ...
    public static com.pullenti.ner.Token _checkApproved(com.pullenti.ner.Token t) {
        ...
        for (t = t.getNext(); t != null; t = t.getNext()) {
            ...
            com.pullenti.ner.Token tt = _checkApproved(t);
            if (tt != null) {
                if (!tt.isNewlineBefore() && com.pullenti.unisharp.Utils.stringsNe(tt.getNormalCaseText(null, false, com.pullenti.morph.MorphGender.UNDEFINED, false), t0.getNormalCaseText(null, false, com.pullenti.morph.MorphGender.UNDEFINED, false))) {
                    t1 = (tt = t);
                    continue;
                }
            }
    ...
    public static InstrToken _new1524(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, ILTypes _arg3, Object _arg4) {
    ...
    public static InstrToken _new1525(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, ILTypes _arg3) {
        InstrToken res = new InstrToken(_arg1, _arg2);
        res.typ = _arg3;
        return res;
    }

    public static InstrToken _new1528(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, ILTypes _arg3, String _arg4) {
        InstrToken res = new InstrToken(_arg1, _arg2);
        res.typ = _arg3;
        res.value = _arg4;
        return res;
    ...
    public static InstrToken _new1530(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, ILTypes _arg3, Object _arg4, String _arg5) {
        InstrToken res = new InstrToken(_arg1, _arg2);
        res.typ = _arg3;
        res.ref = _arg4;
        res.value = _arg5;
    ...
    static {
        m_Directives = new java.util.ArrayList<String>(java.util.Arrays.asList(new String[] {"ПРИКАЗЫВАТЬ", "ПРИКАЗАТЬ", "ОПРЕДЕЛЯТЬ", "ОПРЕДЕЛИТЬ", "ПОСТАНОВЛЯТЬ", "ПОСТАНОВИТЬ", "УСТАНОВИТЬ", "РЕШИЛ", "РЕШИТЬ", "ПРОСИТЬ", "ПРИГОВАРИВАТЬ", "ПРИГОВОРИТЬ", "НАКАЗУВАТИ", "ВИЗНАЧАТИ", "ВИЗНАЧИТИ", "УХВАЛЮВАТИ", "ПОСТАНОВЛЯТИ", "ПОСТАНОВИТИ", "ВСТАНОВИТИ", "ВИРІШИВ", "ВИРІШИТИ", "ПРОСИТИ", "ПРИМОВЛЯТИ", "ЗАСУДИТИ"}));
        m_DirectivesNorm = new java.util.ArrayList<String>(java.util.Arrays.asList(new String[] {"ПРИКАЗ", "ПРИКАЗ", "ОПРЕДЕЛЕНИЕ", "ОПРЕДЕЛЕНИЕ", "ПОСТАНОВЛЕНИЕ", "ПОСТАНОВЛЕНИЕ", "УСТАНОВЛЕНИЕ", "РЕШЕНИЕ", "РЕШЕНИЕ", "ЗАЯВЛЕНИЕ", "ПРИГОВОР", "ПРИГОВОР", "НАКАЗ", "УХВАЛА", "УХВАЛА", "ПОСТАНОВА", "ПОСТАНОВА", "ПОСТАНОВА", "ВСТАНОВЛЕННЯ", "РІШЕННЯ", "РІШЕННЯ", "ЗАЯВА", "ВИРОК", "ВИРОК"}));
    }
}
