...
public class TableHelper {
    ...
    public static java.util.ArrayList<TableRowToken> tryParseRows(com.pullenti.ner.Token t, int maxChar, boolean mustBeStartOfTable) {
        ...
        com.pullenti.unisharp.Outargwrapper<Boolean> wrapisTab548 = new com.pullenti.unisharp.Outargwrapper<Boolean>(isTab);
        TableRowToken rw = parse(t, maxChar, null, wrapisTab548);
        isTab = (wrapisTab548.value != null ? wrapisTab548.value : false);
        if (rw == null) 
            return null;
        ...
        for (t = rw.getEndToken().getNext(); t != null; t = t.getNext()) {
            com.pullenti.unisharp.Outargwrapper<Boolean> wrapisTab547 = new com.pullenti.unisharp.Outargwrapper<Boolean>(isTab);
            TableRowToken rw0 = parse(t, maxChar, rw, wrapisTab547);
            isTab = (wrapisTab547.value != null ? wrapisTab547.value : false);
            if (rw0 == null) 
                break;
            res.add((rw = rw0));
            t = rw0.getEndToken();
    ...
    private static boolean _containsTableChar(com.pullenti.ner.MetaToken mt) {
        for (com.pullenti.ner.Token tt = mt.getBeginToken(); tt != null && tt.endChar <= mt.endChar; tt = tt.getNext()) {
            ...
            else if (((tt.isTableControlChar() && tt.getPrevious() != null && !tt.getPrevious().isTableControlChar()) && tt.getNext() != null && !tt.getNext().isTableControlChar()) && tt.getPrevious().beginChar >= mt.beginChar && tt.getNext().endChar <= mt.endChar) 
                return true;
        }
        return false;
    }
    ...
    private static TableRowToken parse(com.pullenti.ner.Token t, int maxChar, TableRowToken prev, com.pullenti.unisharp.Outargwrapper<Boolean> isTab) {
        ...
        res.cells.add(TableCellToken._new549(t, tt, cellInfo.rowSpan, cellInfo.colSpan));
        for (tt = tt.getNext(); tt != null && ((tt.endChar <= maxChar || maxChar == 0)); tt = tt.getNext()) {
            ...
            if (cellInfo.typ == TableTypes.ROWEND) {
                if (tt != t0) 
                    res.cells.add(TableCellToken._new549(t0, tt, cellInfo.rowSpan, cellInfo.colSpan));
                res.setEndToken(tt);
                res.eor = true;
                break;
            }
