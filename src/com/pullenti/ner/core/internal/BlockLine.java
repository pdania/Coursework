...
public class BlockLine extends com.pullenti.ner.MetaToken {
    ...
    public static BlockLine create(com.pullenti.ner.Token t, com.pullenti.ner.core.TerminCollection names) {
        ...
        int nums = 0;
        while (t != null && t.getNext() != null && t.endChar <= res.endChar) {
        ...
        if (tok != null) {
            ...
            if (com.pullenti.morph.MorphLang.ooNoteq(t.kit.baseLanguage, t.getMorph().getLanguage())) 
                tok = null;
        ...
        if ((((t1 instanceof com.pullenti.ner.NumberToken) || t1.isChar('.'))) && t1.getPrevious() != null) {
            ...
            if (t1.isChar('.')) {
                ...
                for (; t1 != null && t1.beginChar > res.beginChar; t1 = t1.getPrevious()) {
                    if (!t1.isChar('.')) 
                        break;
                }
            }
        ...
        res.isAllUpper = true;
        for (; t != null && t.endChar <= t1.endChar; t = t.getNext()) {
        ...
        if (res.typ == BlkTyps.UNDEFINED) {
            com.pullenti.ner.core.NounPhraseToken npt = com.pullenti.ner.core.NounPhraseHelper.tryParse((res.numberEnd == null ? res.getBeginToken() : res.numberEnd.getNext()), com.pullenti.ner.core.NounPhraseParseAttr.NO, 0);
            if (npt != null) {
                if (npt.noun.isValue("ХАРАКТЕРИСТИКА", null) || npt.noun.isValue("СОДЕРЖАНИЕ", "ЗМІСТ")) {
                    boolean ok = true;
                    for (com.pullenti.ner.Token tt = npt.getEndToken().getNext(); tt != null && tt.endChar <= res.endChar; tt = tt.getNext()) {
                        if (tt.isChar('.')) 
                            continue;
                ...
                if (res.typ == BlkTyps.UNDEFINED && npt != null && npt.endChar <= res.endChar) {
                    ...
                    if (ok) {
                        if (npt.getBeginToken() == npt.getEndToken() && npt.noun.isValue("СПИСОК", null) && npt.endChar == res.endChar) 
                            ok = false;
                        for (com.pullenti.ner.Token tt = npt.getEndToken().getNext(); tt != null && tt.endChar <= res.endChar; tt = tt.getNext()) {
                        ...
                        if (ok) {
                            ...
                            res.isExistName = true;
                            if (publ == 0 && (res.endChar < (((res.kit.getSofa().getText().length() * 2) / 3)))) {
                                if (res.numberEnd != null) 
    ...
    public static void initialize() {
        ...
        for (String s : new String[] {"СОДЕРЖАНИЕ", "СОДЕРЖИМОЕ", "ОГЛАВЛЕНИЕ", "ПЛАН", "PLAN", "ЗМІСТ", "CONTENTS", "INDEX"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.INDEX));
        }
        for (String s : new String[] {"ГЛАВА", "CHAPTER", "РАЗДЕЛ", "ПАРАГРАФ", "VOLUME", "SECTION", "РОЗДІЛ"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.CHAPTER));
        }
        for (String s : new String[] {"ВВЕДЕНИЕ", "ВСТУПЛЕНИЕ", "ПРЕДИСЛОВИЕ", "INTRODUCTION"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.INTRO));
        }
        for (String s : new String[] {"ВСТУП", "ПЕРЕДМОВА"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new472(s, com.pullenti.morph.MorphLang.UA, BlkTyps.INTRO));
        }
        for (String s : new String[] {"ВЫВОДЫ", "ВЫВОД", "ЗАКЛЮЧЕНИЕ", "CONCLUSION", "ВИСНОВОК", "ВИСНОВКИ"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.CONSLUSION));
        }
        for (String s : new String[] {"ПРИЛОЖЕНИЕ", "APPENDIX", "ДОДАТОК"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.APPENDIX));
        }
        for (String s : new String[] {"СПИСОК СОКРАЩЕНИЙ", "СПИСОК УСЛОВНЫХ СОКРАЩЕНИЙ", "СПИСОК ИСПОЛЬЗУЕМЫХ СОКРАЩЕНИЙ", "УСЛОВНЫЕ СОКРАЩЕНИЯ", "ОБЗОР ЛИТЕРАТУРЫ", "АННОТАЦИЯ", "ANNOTATION", "БЛАГОДАРНОСТИ", "SUPPLEMENT", "ABSTRACT", "СПИСОК СКОРОЧЕНЬ", "ПЕРЕЛІК УМОВНИХ СКОРОЧЕНЬ", "СПИСОК ВИКОРИСТОВУВАНИХ СКОРОЧЕНЬ", "УМОВНІ СКОРОЧЕННЯ", "ОГЛЯД ЛІТЕРАТУРИ", "АНОТАЦІЯ", "ПОДЯКИ"}) {
            m_Ontology.add(com.pullenti.ner.core.Termin._new135(s, BlkTyps.MISC));
        }
    }
    public BlockLine() {
        super();
