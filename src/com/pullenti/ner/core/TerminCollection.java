...
public class TerminCollection {
    ...
    private void _AddToTree(String key, Termin t) {
        ...
        for (int i = 0; i < key.length(); i++) {
            ...
            CharNode nn;
            com.pullenti.unisharp.Outargwrapper<CharNode> wrapnn658 = new com.pullenti.unisharp.Outargwrapper<CharNode>();
            boolean inoutres659 = com.pullenti.unisharp.Utils.tryGetValue(nod.children, ch, wrapnn658);
            nn = wrapnn658.value;
            if (!inoutres659) 
    ...
    private void _RemoveFromTree(String key, Termin t) {
        ...
        for (int i = 0; i < key.length(); i++) {
            ...
            if (nod.children == null) 
                return;
            CharNode nn;
            com.pullenti.unisharp.Outargwrapper<CharNode> wrapnn660 = new com.pullenti.unisharp.Outargwrapper<CharNode>();
            boolean inoutres661 = com.pullenti.unisharp.Utils.tryGetValue(nod.children, ch, wrapnn660);
            nn = wrapnn660.value;
            if (!inoutres661) 
                return;
            nod = nn;
    ...
    private java.util.ArrayList<Termin> _FindInTree(String key, com.pullenti.morph.MorphLang lang) {
        ...
        for (int i = 0; i < key.length(); i++) {
            ...
            CharNode nn = null;
            if (nod.children != null) {
                com.pullenti.unisharp.Outargwrapper<CharNode> wrapnn662 = new com.pullenti.unisharp.Outargwrapper<CharNode>();
                com.pullenti.unisharp.Utils.tryGetValue(nod.children, ch, wrapnn662);
                nn = wrapnn662.value;
            }
            if (nn == null) {
                if (ch == ((short)32)) {
    ...
    private void addToHash1(short key, Termin t) {
        ...
        com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>> wrapli663 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>>();
        boolean inoutres664 = com.pullenti.unisharp.Utils.tryGetValue(m_Hash1, key, wrapli663);
        li = wrapli663.value;
        if (!inoutres664) 
    ...
    public TerminToken tryParse(com.pullenti.ner.Token token, TerminParseAttr pars) {
        if (termins.size() == 0) 
            return null;
        java.util.ArrayList<TerminToken> li = this.tryParseAll(token, pars, 0.0);
        if (li != null) 
            return li.get(0);
        else 
            return null;
     ...
     * @return 
     */
    public java.util.ArrayList<TerminToken> tryParseAll(com.pullenti.ner.Token token, TerminParseAttr pars, double simD) {
        if (token == null) 
            return null;
        if ((simD < 1) && simD > 0.05) 
            return this._TryAttachAllSim(token, simD);
    ...
    private java.util.ArrayList<TerminToken> _TryAttachAllSim(com.pullenti.ner.Token token, double simD) {
        if ((simD >= 1 || (simD < 0.05) || termins.size() == 0) || token == null) 
            return null;
        ...
        for (Termin t : termins) {
            ...
            TerminToken ar = t.tryParse(tt, TerminParseAttr.NO, simD);
            if (ar == null) 
                continue;
            ar.termin = t;
    ...
    private java.util.ArrayList<TerminToken> _TryAttachAll_(com.pullenti.ner.Token token, TerminParseAttr pars, boolean mainRoot) {
        ...
        if (tt != null) {
            ...
            if ((((pars.value()) & (TerminParseAttr.TERMONLY.value()))) != (TerminParseAttr.NO.value())) {
                ...
                for (int i = 0; i < tt.invariantPrefixLength; i++) {
                    ...
                    CharNode nn;
                    com.pullenti.unisharp.Outargwrapper<CharNode> wrapnn665 = new com.pullenti.unisharp.Outargwrapper<CharNode>();
                    boolean inoutres666 = com.pullenti.unisharp.Utils.tryGetValue(nod.children, ch, wrapnn665);
                    nn = wrapnn665.value;
                    if (!inoutres666) {
                        noVars = true;
                        break;
            ...
            if (!noVars) {
                com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>> wrapres671 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>>(res);
                boolean inoutres672 = this._manageVar(token, pars, s, nod, len0, wrapres671);
                ...
                for (int i = 0; i < tt.getMorph().getItemsCount(); i++) {
                    ...
                    if (ok) {
                        com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>> wrapres667 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>>(res);
                        boolean inoutres668 = this._manageVar(token, pars, wf.normalCase, nod, (int)tt.invariantPrefixLength, wrapres667);
                    ...
                    if (j < i) 
                        continue;
                    com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>> wrapres669 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>>(res);
                    boolean inoutres670 = this._manageVar(token, pars, wf.normalFull, nod, (int)tt.invariantPrefixLength, wrapres669);
            ...
            com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>> wrapres673 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>>(res);
            boolean inoutres674 = this._manageVar(token, pars, (((com.pullenti.ner.NumberToken)com.pullenti.unisharp.Utils.cast(token, com.pullenti.ner.NumberToken.class))).getValue().toString(), root, 0, wrapres673);
        ...
        if (!wasVars && s != null && s.length() == 1) {
            ...
            com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>> wrapvars675 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>>();
            boolean inoutres676 = com.pullenti.unisharp.Utils.tryGetValue(m_Hash1, (short)s.charAt(0), wrapvars675);
            vars = wrapvars675.value;
            if (inoutres676) {
                for (Termin t : vars) {
                    if (!t.lang.isUndefined()) {
                        if (!token.getMorph().getLanguage().isUndefined()) {
                    ...
                    TerminToken ar = t.tryParse(tt, TerminParseAttr.NO, 0.0);
                    if (ar == null) 
                        continue;
                    ar.termin = t;
                    if (res == null) {
    ...
    private boolean _manageVar(com.pullenti.ner.Token token, TerminParseAttr pars, String v, CharNode nod, int i0, com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<TerminToken>> res) {
        for (int i = i0; i < v.length(); i++) {
            ...
            if (nod.children == null) 
                return false;
            CharNode nn;
            com.pullenti.unisharp.Outargwrapper<CharNode> wrapnn677 = new com.pullenti.unisharp.Outargwrapper<CharNode>();
            boolean inoutres678 = com.pullenti.unisharp.Utils.tryGetValue(nod.children, ch, wrapnn677);
            nn = wrapnn677.value;
            if (!inoutres678) 
                return false;
            nod = nn;
        ...
        if (vars == null || vars.size() == 0) 
            return false;
        for (Termin t : vars) {
            TerminToken ar = t.tryParse(token, pars, 0.0);
            if (ar != null) {
                ar.termin = t;
                if (res.value == null) {
    ...
    public java.util.ArrayList<Termin> findTerminByCanonicText(String text) {
        if (m_HashCanonic == null) {
            ...
            for (Termin t : termins) {
                ...
                java.util.ArrayList<Termin> li;
                com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>> wrapli679 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>>();
                boolean inoutres680 = com.pullenti.unisharp.Utils.tryGetValue(m_HashCanonic, ct, wrapli679);
                li = wrapli679.value;
                if (!inoutres680) 
        ...
        com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>> wrapres681 = new com.pullenti.unisharp.Outargwrapper<java.util.ArrayList<Termin>>();
        boolean inoutres682 = com.pullenti.unisharp.Utils.tryGetValue(m_HashCanonic, text, wrapres681);
