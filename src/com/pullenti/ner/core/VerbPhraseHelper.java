...
public class VerbPhraseHelper {
    ...
    private static VerbPhraseToken tryParseRu(com.pullenti.ner.Token t, boolean canBePartition) {
        ...
        for (; t != null; t = t.getNext()) {
            ...
            else if ((mc.isAdjective() && tt.getMorph().containsAttr("к.ф.", null) && tt.term.endsWith("О")) && NounPhraseHelper.tryParse(tt, NounPhraseParseAttr.NO, 0) == null) 
                ty = 2;
            else if (mc.isAdjective() && canBePartition) {
            ...
            res.setEndToken(t);
            VerbPhraseItemToken it = VerbPhraseItemToken._new683(t, t, new com.pullenti.ner.MorphCollection(t.getMorph()));
            if (not != null) {
            ...
            if (norm == null) {
                ...
                if (ty == 1 && !tt.getMorph().getCase().isUndefined()) {
                    com.pullenti.morph.MorphWordForm mi = com.pullenti.morph.MorphWordForm._new684(com.pullenti.morph.MorphCase.NOMINATIVE, com.pullenti.morph.MorphNumber.SINGULAR, com.pullenti.morph.MorphGender.MASCULINE);
