...
public class MeasureToken extends com.pullenti.ner.MetaToken {
    ...
    public static MeasureToken tryParseMinimal(com.pullenti.ner.Token t, com.pullenti.ner.core.TerminCollection addUnits, boolean canOmitNumber) {
        if (t == null || (t instanceof com.pullenti.ner.ReferentToken)) 
            return null;
        java.util.ArrayList<NumbersWithUnitToken> mt = NumbersWithUnitToken.tryParseMulti(t, addUnits, canOmitNumber, false, false, false);
        if (mt == null) 
            return null;
        ...
        MeasureToken res;
        if (mt.size() == 1) {
            res = _new1625(mt.get(0).getBeginToken(), mt.get(mt.size() - 1).getEndToken(), mt.get(0));
        ...
        for (NumbersWithUnitToken m : mt) {
            res.internals.add(_new1625(m.getBeginToken(), m.getEndToken(), m));
        }
        return res;
    }
    ...
    private void _parseInternals(com.pullenti.ner.core.TerminCollection addUnits) {
        if (getEndToken().getNext() != null && ((this.getEndToken().getNext().isCharOf("\\/") || this.getEndToken().getNext().isValue("ПРИ", null)))) {
            ...
            if (mt1 != null) {
                ...
                NumbersWithUnitToken mt = NumbersWithUnitToken.tryParse(this.getEndToken().getNext().getNext(), addUnits, false, false, false, false);
                if (mt != null && mt.units.size() > 0 && !UnitToken.canBeEquals(nums.units, mt.units)) {
                    internals.add(_new1625(mt.getBeginToken(), mt.getEndToken(), mt));
    ...
    public static MeasureToken tryParse(com.pullenti.ner.Token t, com.pullenti.ner.core.TerminCollection addUnits, boolean canBeSet, boolean canUnitsAbsent, boolean isResctriction, boolean isSubval) {
        ...
        int minmax = 0;
        com.pullenti.unisharp.Outargwrapper<Integer> wrapminmax1638 = new com.pullenti.unisharp.Outargwrapper<Integer>(minmax);
        com.pullenti.ner.Token tt = NumbersWithUnitToken._isMinOrMax(t0, wrapminmax1638);
        minmax = (wrapminmax1638.value != null ? wrapminmax1638.value : 0);
        if (tt != null) 
            t = tt.getNext();
        ...
        if (npt == null) {
            ...
            com.pullenti.ner.date.internal.DateItemToken dtok = com.pullenti.ner.date.internal.DateItemToken.tryAttach(t, null, false);
            if (dtok != null) 
                return null;
        }
        com.pullenti.ner.Token t1 = npt.getEndToken();
        t = npt.getEndToken();
        com.pullenti.ner.MetaToken _name = com.pullenti.ner.MetaToken._new594(npt.getBeginToken(), npt.getEndToken(), npt.getMorph());
        ...
        for (tt = t1.getNext(); tt != null; tt = tt.getNext()) {
            ...
            if (tt.isTableControlChar()) 
                break;
            com.pullenti.unisharp.Outargwrapper<Integer> wrapminmax1630 = new com.pullenti.unisharp.Outargwrapper<Integer>(minmax);
            com.pullenti.ner.Token tt2 = NumbersWithUnitToken._isMinOrMax(tt, wrapminmax1630);
            minmax = (wrapminmax1630.value != null ? wrapminmax1630.value : 0);
            if (tt2 != null) {
                t1 = (t = (tt = tt2));
                continue;
            }
            ...
            if (tt.isValue("ПРИ", null)) {
                ...
                NumbersWithUnitToken n1 = NumbersWithUnitToken.tryParse(tt.getNext(), addUnits, false, false, false, false);
                if (n1 != null && n1.units.size() > 0) {
                    mt1 = _new1625(n1.getBeginToken(), n1.getEndToken(), n1);
                    _internals.add(mt1);
                    t1 = (t = (tt = mt1.getEndToken()));
                    continue;
                }
            ...
            if (mt0 != null) {
                com.pullenti.ner.core.NounPhraseToken npt1 = com.pullenti.ner.core.NounPhraseHelper.tryParse(tt, com.pullenti.ner.core.NounPhraseParseAttr.of((com.pullenti.ner.core.NounPhraseParseAttr.PARSENUMERICASADJECTIVE.value()) | (com.pullenti.ner.core.NounPhraseParseAttr.PARSEPREPOSITION.value())), 0);
        ...
        for (t1 = t1.getNext(); t1 != null; t1 = t1.getNext()) {
            if (t1.isTableControlChar()) {
                ...
                if (t1.isChar(':')) {
                    ...
                    if (li.size() > 1) {
                        MeasureToken res0 = _new1631(t0, li.get(li.size() - 1).getEndToken(), li, true);
        ...
        if (t1 == null) 
            return null;
        java.util.ArrayList<NumbersWithUnitToken> mts = NumbersWithUnitToken.tryParseMulti(t1, addUnits, false, not, true, isResctriction);
        if (mts == null) {
            if (units != null && units.size() > 0) {
                if (t1 == null || t1.getPrevious().isChar(':')) {
                    ...
                    if (t1 == null) {
                        ...
                        t1 = t1.getPrevious();
                    mts.add(NumbersWithUnitToken._new1632(t0, t1, Double.NaN));
                }
            }
            if (mts == null) 
                return null;
        ...
        if (mts.size() > 1 && _internals.size() == 0) {
            ...
            MeasureToken res1 = _new1633(t0, mts.get(mts.size() - 1).getEndToken(), _name.getMorph(), true);
            ...
            for (int k = 0; k < mts.size(); k++) {
                MeasureToken ttt = _new1625(mts.get(k).getBeginToken(), mts.get(k).getEndToken(), mts.get(k));
                if (whd != null) {
        ...
        MeasureToken res = _new1635(t0, mt.getEndToken(), _name.getMorph(), _internals);
        ...
        if (t1 != null && t1.isCommaAnd()) 
            t1 = t1.getNext();
        java.util.ArrayList<NumbersWithUnitToken> mts1 = NumbersWithUnitToken.tryParseMulti(t1, addUnits, false, false, false, false);
        if ((mts1 != null && mts1.size() == 1 && (t1.getWhitespacesBeforeCount() < 3)) && mts1.get(0).units.size() > 0 && !UnitToken.canBeEquals(mts.get(0).units, mts1.get(0).units)) {
            res.isSet = true;
            res.nums = null;
            res.internals.add(_new1625(mt.getBeginToken(), mt.getEndToken(), mt));
            res.internals.add(_new1625(mts1.get(0).getBeginToken(), mts1.get(0).getEndToken(), mts1.get(0)));
            res.setEndToken(mts1.get(0).getEndToken());
        }
        return res;
    }

    public static MeasureToken _new1625(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, NumbersWithUnitToken _arg3) {
        MeasureToken res = new MeasureToken(_arg1, _arg2);
        res.nums = _arg3;
        return res;
    }

    public static MeasureToken _new1631(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, java.util.ArrayList<MeasureToken> _arg3, boolean _arg4) {
        MeasureToken res = new MeasureToken(_arg1, _arg2);
        res.internals = _arg3;
        res.isEmpty = _arg4;
        return res;
    ...
    public static MeasureToken _new1633(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, com.pullenti.ner.MorphCollection _arg3, boolean _arg4) {
        MeasureToken res = new MeasureToken(_arg1, _arg2);
        res.setMorph(_arg3);
        res.reliable = _arg4;
        return res;
    ...
    public static MeasureToken _new1635(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, com.pullenti.ner.MorphCollection _arg3, java.util.ArrayList<MeasureToken> _arg4) {
        MeasureToken res = new MeasureToken(_arg1, _arg2);
        res.setMorph(_arg3);
        res.internals = _arg4;
        return res;
