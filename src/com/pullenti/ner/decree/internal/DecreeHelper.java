...
public class DecreeHelper {
    ...
    public static java.time.LocalDateTime parseDateTime(String str) {
        ...
        try {
            ...
            int y;
            com.pullenti.unisharp.Outargwrapper<Integer> wrapy852 = new com.pullenti.unisharp.Outargwrapper<Integer>();
            boolean inoutres853 = com.pullenti.unisharp.Utils.parseInteger(prts[0], 0, null, wrapy852);
            y = (wrapy852.value != null ? wrapy852.value : 0);
            if (!inoutres853) 
                return null;
            int mon = 0;
            int day = 0;
            if (prts.length > 1) {
                com.pullenti.unisharp.Outargwrapper<Integer> wrapmon850 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                boolean inoutres851 = com.pullenti.unisharp.Utils.parseInteger(prts[1], 0, null, wrapmon850);
                mon = (wrapmon850.value != null ? wrapmon850.value : 0);
                if (inoutres851) {
                    if (prts.length > 2) {
                        com.pullenti.unisharp.Outargwrapper<Integer> wrapday849 = new com.pullenti.unisharp.Outargwrapper<Integer>();
                        com.pullenti.unisharp.Utils.parseInteger(prts[2], 0, null, wrapday849);
                        day = (wrapday849.value != null ? wrapday849.value : 0);
                    }
                }
            }
            if (mon <= 0) 
    ...
    public static CanonicDecreeRefUri tryCreateCanonicDecreeRefUri(com.pullenti.ner.Token t) {
        ...
        if (dr != null) {
            if (dr.getKind() == com.pullenti.ner.decree.DecreeKind.PUBLISHER) 
                return null;
            res = CanonicDecreeRefUri._new854(t.kit.getSofa().getText(), dr, t.beginChar, t.endChar);
            ...
            if (rt.getBeginToken().isChar('(') && rt.getEndToken().isChar(')')) {
                res = CanonicDecreeRefUri._new854(t.kit.getSofa().getText(), dr, rt.getBeginToken().getNext().beginChar, rt.getEndToken().getPrevious().endChar);
                return res;
            }
        ...
        res = CanonicDecreeRefUri._new856(t.kit.getSofa().getText(), dpr, t.beginChar, t1.endChar, hasDiap);
    ...
    public static com.pullenti.ner.MetaToken checkNds(com.pullenti.ner.Token t, double nds, boolean ndsMustbeMoney) {
                ...
                if (ne != null && ne.exTyp == com.pullenti.ner.core.NumberExType.PERCENT) {
                    if (Math.abs(ne.getRealValue() - nds) > 0.0001) {
                        ...
                        if (hasNds) 
                            ok = true;
                        if (ok) 
                            return com.pullenti.ner.MetaToken._new857(tt, ne.getEndToken(), ("Размер НДС должен быть " + nds + "%, а не " + ne.getRealValue() + "%"));
                    }
        ...
        if (m1 == null) {
            if (ndsMustbeMoney) 
                return com.pullenti.ner.MetaToken._new857(ndsT0, ndsT1, "Размер НДС должен быть в денежном выражении");
            return null;
        }
        if (hasAll) 
            return null;
        ...
        if (delta > 0.011) {
            if ((delta < 1) && m1.getRest() == 0 && m.getRest() == 0) {
                ...
                com.pullenti.ner.money.MoneyReferent mr = com.pullenti.ner.money.MoneyReferent._new859(m1.getCurrency(), mustBe);
                return com.pullenti.ner.MetaToken._new857(t, tt, ("Размер НДС должен быть " + _outMoney(mr) + ", а не " + _outMoney(m1)));
            }
        }
        if (incl) 
            return null;
        ...
        if (m2 != null && hasAll) {
            ...
            if (delta > 0.01) {
                ...
                return com.pullenti.ner.MetaToken._new857(t, tt, err);
            }
        }
        return null;
    }
