...
public class MailAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public java.util.Collection<com.pullenti.ner.ReferentClass> getTypeSystem() {
        return java.util.Arrays.asList(new com.pullenti.ner.ReferentClass[] {com.pullenti.ner.mail.internal.MetaLetter.globalMeta});
    }
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        ...
        for (i = 0; i < lines.size(); i++) {
            ...
            if (ml.typ == com.pullenti.ner.mail.internal.MailLine.Types.FROM) {
                ...
                if (!isNew) {
                    for (com.pullenti.ner.Token tt = ml.getBeginToken(); tt != null && tt.endChar <= ml.endChar; tt = tt.getNext()) {
        ...
        for (int j = 0; j < blocks.size(); j++) {
            ...
            if (lines.get(0).typ == com.pullenti.ner.mail.internal.MailLine.Types.FROM) {
                ...
                MailReferent _mail = MailReferent._new1618(MailKind.HEAD);
            ...
            for (int ii = i0; ii < lines.size(); ii++) {
                if (lines.get(ii).typ == com.pullenti.ner.mail.internal.MailLine.Types.HELLO) {
                    MailReferent _mail = MailReferent._new1618(MailKind.HELLO);
            ...
            if (i0 < lines.size()) {
                if (t2 != null && t2.getPrevious() == null) {
                    ...
                    MailReferent _mail = MailReferent._new1618(MailKind.BODY);
                ...
                if (t2 != null) {
                    MailReferent _mail = MailReferent._new1618(MailKind.TAIL);
                    ...
                    for (i = i0; i < lines.size(); i++) {
                        if (lines.get(i).beginChar >= t2.beginChar) {
